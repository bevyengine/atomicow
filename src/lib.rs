...
///
/// This means that static references should prefer [`CowArc::from`] or [`CowArc::Static`] and non-static references must
/// use [`CowArc::Borrowed`].
...
impl<T: ?Sized> CowArc<'static, T> {
    ...
    /// It is only possible to call this method if `'a` is `'static`.
